...
class BitPacker(object):
    ...
    def pack(self, codepoints):
        ...
        >>> [ b for b in pkr.pack([ 1, 257]) ] == [ int2byte(0), int2byte(0xC0), int2byte(0x40) ]
        True
        """
        tailbits = []
...
class BitUnpacker(object):
    ...
    def unpack(self, bytesource):
        ...
        >>> [ i for i in unpk.unpack([ int2byte(0), int2byte(0xC0), int2byte(0x40) ]) ]
        [1, 257]
        """
        bits = []
        offset = 0
        ...
        if codepoint == CLEAR_CODE:
            ...
            if codepoint in self._codepoints:
                ...
                if None != self._prefix:
                    self._codepoints[ len(self._codepoints) ] = self._prefix + int2byte(ret[0])

            else:
                ret = self._prefix + int2byte(self._prefix[0])
...
class Encoder(object):
    ...
    def _encode_byte(self, point):
        ...
        byte = point if isinstance(point, bytes) else int2byte(point)
...
def readbytes(filename, buffersize=1024):
    ...
    with open(filename, "rb") as infile:
        for byte in iter(filebytes(infile, buffersize)):
            yield int2byte(byte)  # TODO optimize, we are re-casting to bytes
